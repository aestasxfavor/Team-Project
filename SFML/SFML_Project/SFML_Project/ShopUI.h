...
public:
	...
	function<void(int)> onSelectCallback;       // 콜백함수 정의
	function<void()> onCloseCallback;  //


	void SetOnSelect(function<void(int)> callback);   // 콜백 함수 등록
	void SetOnClose(function<void()> callback);		  // 콜백 함수 등록

	const vector<PlayerStats::StatOption>& GetSelectedChoices() const;	// 

	void ClearSelectedChoices();	// 선택 초기화
